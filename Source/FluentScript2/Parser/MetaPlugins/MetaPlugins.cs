...
namespace ComLib.Lang.Parsing.MetaPlugins
{
	...
	*/
	...
	{
		private readonly IDictionary<string, List<CompilerPlugin>> _pluginExprs;
		private readonly IDictionary<string, List<CompilerPlugin>> _pluginTokens;
		private readonly IDictionary<string, List<CompilerPlugin>> _pluginLexTokens;
		private readonly List<CompilerPlugin> _allPlugins;
		private const string TypeDateToken = "$DateToken";
		private const string TypeNumberToken = "$NumberToken";
		private const string TypeIdentSymbolToken = "$IdentSymbolToken";
		...
		public bool CanHandleExp(Token token)
		{
			...
			{
				if (_pluginExprs.ContainsKey(TypeNumberToken))
					plugins = _pluginExprs[TypeNumberToken];
			}
			else if (token.Kind == TokenKind.Ident)
			{
				if (_pluginExprs.ContainsKey(TypeIdentSymbolToken))
				{
					if (Symbols.Contains(token.Text))
					{
						plugins = _pluginExprs[TypeIdentSymbolToken];
					}
				}
			}
			if (plugins == null)
		...
		public bool CanHandleTok(Token token, bool isCurrent)
		{
			...
			{
				if (_pluginTokens.ContainsKey(TypeDateToken))
					plugins = _pluginTokens[TypeDateToken];
			}
			if (plugins == null)
				return false;
