...
namespace ComLib.Lang.Parsing
{
	...
	public class Parser : ParserBase
	{
		...
		public override void Init(string script, Memory memory)
		{
			...
			Tokenize();
			SetupTokenIteratorReferences(_tokenIt);
		...
		public Expr ParseStatement()
		{
			...
			while (stmt == null && !_tokenIt.IsEnded)
			{
				...
				{
					stmt = _context.PluginsMeta.ParseExp(OnDemandEvaluator);
				}
		...
		private Expr ParseSystemStatement()
		{
			...
			if (plugin is IParserCallbacks parserCallbacks)
				parserCallbacks.OnParseComplete(stmt);
		...
		private Expr ParseExtensionStatement()
		{
			...
			if (!expPlugin.IsAssignmentSupported)
			{
				...
				if (expPlugin is IParserCallbacks parserCallbacks)
					(parserCallbacks).OnParseComplete(result);
		...
		{
			var plugin = Context.Plugins.GetSysStmt(Tokens.Var) as VarPlugin;
		...
		public Expr ParseFuncExpression(Expr nameExp, List<string> members)
		{
			...
			{
				var symScope = _context.Symbols.Current;
		...
		public Expr ParseIdExpression(string name, Expr existing, bool isCurrentTokenAnOperator)
		{
			...
			{
				return ParseBindingCallExpr("sys", "compiler");
			}
		...
		public void SetupContext(Expr expr, TokenData token)
		{
			...
			if (expr.Ref == null) expr.Ref = new ScriptRef(ScriptName, reftoken.Line, reftoken.LineCharPos);
		}
