...
namespace ComLib.Lang.Helpers
{
	...
	public static class DateTimeTypeHelper
	{
		...
		public static Tuple<TimeSpan, bool, string> ParseTime(string strTime)
		{
			...
			var minutes = 0;
			var seconds = 0;
			if (!string.IsNullOrEmpty(strhours) && !int.TryParse(strhours, out hours))
			{
				return new Tuple<TimeSpan, bool, string>(TimeSpan.MinValue, false, "Hours are invalid.");
			}
			if (!string.IsNullOrEmpty(strminutes) && !int.TryParse(strminutes, out minutes))
			{
				return new Tuple<TimeSpan, bool, string>(TimeSpan.MinValue, false, "Minutes are invalid.");
			}
			if (!string.IsNullOrEmpty(strseconds) && !int.TryParse(strseconds, out seconds))
			{
		...
		public static DateTime CreateDateFrom(object[] parameters)
		{
			...
			// Case 1: From string
			if (parameters.Length == 1 && parameters[0] is string @string)
			{
			...
			// Case 2: From Date
			if (parameters.Length == 1 && parameters[0] is DateTime time)
			{
				var d = time;
